"""FGN v5 — Hierarchical Geometric Routing training script.

Single-phase training (no Phase 0/1 distinction).
Sharpness annealing for soft escalation mode.
Logs escalation rate, entropy, and threshold per layer.
"""
...
def create_model(config: FGNConfig, device: torch.device):
    ...
    if config.model_type == "flat":
        ...
        return FGNv5Model(config).to(device)
...
def print_v5_status(model, result, step, tok_per_sec, config):
    """Print compact v5 status line."""
    m = model._orig_mod if hasattr(model, '_orig_mod') else model
    is_v5 = isinstance(m, FGNv5Model)
    ...
    if is_v5:
        ...
        log_t_str = (f"\n  log_t=[{','.join(f'{t:.2f}' for t in log_ts)}]")

        if hasattr(m.layers[0], 'threshold_raw'):
            thresholds = [torch.sigmoid(l.threshold_raw).item() for l in m.layers]
            thresh_str = (f"\n  threshold=[{','.join(f'{t:.2f}' for t in thresholds)}]")

    esc_penalty = result.get("esc_penalty", torch.tensor(0.0)).item()

    # Determine sharpness
    sharp_str = ""
    if is_v5 and hasattr(m.layers[0], 'sharpness'):
        sharp_str = f", sharp={m.layers[0].sharpness:.1f}"

    print(f"  [step={step}] loss={result['loss'].item():.4f}, "
          f"ce={result['ce_loss'].item():.4f}, "
          f"esc_p={esc_penalty:.4f}, "
          f"cv={result['metric_cv'].item():.4f}, "
          f"|k|={result['avg_kappa'].item():.4f}, "
          f"tok/s={tok_per_sec:.0f}{sharp_str}"
          f"{esc_str}{ent_str}{log_t_str}{thresh_str}")
...
def train(args, config, device):
    """Single-phase training with sharpness annealing."""
    tokenizer = _get_tokenizer()
    task_kwargs = json.loads(args.task_kwargs)

    print(f"\n{'='*70}")
    print(f"FGN v5 Training — {config.model_type}")
    print(f"{'='*70}")

    model = create_model(config, device)
    is_v5 = isinstance(model, FGNv5Model)

    n_params = sum(p.numel() for p in model.parameters())
    print(f"  Parameters: {n_params:,}")
    if is_v5:
        print(f"  Architecture: v5 hierarchical (GeoRoute → entropy escalation → Attention)")
        print(f"  Metric type: {config.geo_metric_type}")
        print(f"  Escalation mode: {config.escalation_mode}")
    else:
    ...
    for step in range(total_steps):
        # Sharpness annealing (soft escalation mode)
        if is_v5 and config.escalation_mode == "soft":
            if step < config.escalation_sharpness_steps:
                progress = step / max(1, config.escalation_sharpness_steps)
                sharpness = (config.escalation_sharpness_init +
                             progress * (config.escalation_sharpness_final -
                                         config.escalation_sharpness_init))
            else:
                sharpness = config.escalation_sharpness_final
            model.set_sharpness(sharpness)
        ...
        if step % args.log_every == 0:
            ...
            print_v5_status(model, result, step, tok_s, config)
    ...
    if is_v5:
        ...
        for i, layer in enumerate(m.layers):
            parts = [f"Layer {i}: log_t={layer.geo_route.log_t.item():.4f}"]
            if hasattr(layer, 'threshold_raw'):
                parts.append(f"threshold={torch.sigmoid(layer.threshold_raw).item():.4f}")
            print(f"    {', '.join(parts)}")
...
def main():
    parser = argparse.ArgumentParser(description="FGN v5 Training")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--task", type=str, default="W",
                        help="Task name (default: W = gridworld)")
    parser.add_argument("--output_dir", type=str, default="output_v5")
    ...
    if config.model_type == "fgn":
        print(f"Metric type: {config.geo_metric_type}")
        print(f"Escalation: {config.escalation_mode}, "
              f"sharpness {config.escalation_sharpness_init}→{config.escalation_sharpness_final} "
              f"over {config.escalation_sharpness_steps} steps")
