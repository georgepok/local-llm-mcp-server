"""FGN v7 — Sandwich architecture training script.

Supports v7 sandwich, v6 budget-based, and flat models.
No sharpness annealing. No phased training.
"""
...
def train(args, config, device):
    ...
    print(f"\n{'='*70}")
    print(f"FGN v7 Training — {config.model_type}")
    print(f"{'='*70}")
    ...
    if is_v7:
        print(f"  Architecture: v7 sandwich ({config.sandwich_bottom_geo_layers} geo + "
              f"{config.sandwich_middle_attn_layers} attn + "
              f"{config.sandwich_top_geo_layers} geo)")
    ...
    for step in range(total_steps):
        ...
        if step % args.log_every == 0:
            ...
            print_v7_status(model, result, step, tok_s)
            ...
            if is_v6:
                writer.add_scalar("escalation/rate", result["escalation_rate"].item(), step)
                writer.add_scalar("escalation/entropy", result["avg_entropy"].item(), step)
    ...
    if is_v7:
        ...
        for i, layer in enumerate(m.bottom_geo):
            log_t = layer.geo_route.log_t
            print(f"    Bottom geo {i}: log_t=[{','.join(f'{t:.2f}' for t in log_t.tolist())}]")
        for i, layer in enumerate(m.middle_attn):
            print(f"    Middle attn {i}: (standard attention)")
        for i, layer in enumerate(m.top_geo):
            log_t = layer.geo_route.log_t
            print(f"    Top geo {i}: log_t=[{','.join(f'{t:.2f}' for t in log_t.tolist())}]")
    elif is_v6:
        print(f"\n  Final per-layer summary:")
...
def main():
    parser = argparse.ArgumentParser(description="FGN v7 Training")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--task", type=str, default="W")
    parser.add_argument("--output_dir", type=str, default="output_v7")
    ...
    if config.model_type == "fgn":
        ...
        elif hasattr(config, 'attention_budgets'):
