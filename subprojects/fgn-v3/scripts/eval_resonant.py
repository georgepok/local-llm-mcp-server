"""Resonant Geometry — Evaluation script.

Evaluates FluidNet (with/without structural energy) and flat models
on continuous gridworld with CV/kappa tracking for flattening analysis.
"""
...
def evaluate_cw(model, tokenizer, config, device,
    ...
    return {
        ...
        "structural_energy": e_struct_sum / max(n_batches, 1),
...
EVAL_CONDITIONS = [
    # (name, kwargs, n_batches_override)
    ...
    ("Near: 15-20rm [0,100] R=30",
     dict(n_rooms_min=15, n_rooms_max=20, space_size=100.0, connect_radius=30.0,
          min_steps=4, max_steps=10, min_state_changes=1),
     100),  # more batches — fewer valid sequences per batch at this size
    ...
    ("Far: 18-20rm [0,150] R=40",
     dict(n_rooms_min=18, n_rooms_max=20, space_size=150.0, connect_radius=40.0,
          min_steps=4, max_steps=10, min_state_changes=1),
     100),  # softened from 20-25rm [0,200] R=50 to fit in seq_len=1024
]


def main():
    parser = argparse.ArgumentParser(description="Resonant Geometry Eval")
    ...
    if config.architecture_version == "fluid":
        variant = f"FluidNet (λ={config.structural_energy_lambda})"
    ...
    print(f"{'Condition':<28} {'SeqAcc':>8} {'TokAcc':>8} {'NavAcc':>8} "
          ...
          f"{'|κ|':>8} {'E_struct':>8} {'t_loc':>6} {'t_med':>6} {'t_glo':>6} {'Seqs':>6}")
    print(f"{'-'*28} {'-'*8} {'-'*8} {'-'*8} {'-'*8} {'-'*10} {'-'*8} "
          f"{'-'*8} {'-'*8} {'-'*6} {'-'*6} {'-'*6} {'-'*6}")

    for name, kwargs, n_batches_override in EVAL_CONDITIONS:
        nb = n_batches_override or args.n_batches
        results = evaluate_cw(
            model, tokenizer, config, device,
            n_batches=nb, batch_size=args.batch_size,
            **kwargs,
        )
        print(f"{name:<28} "
              ...
              f"{results['structural_energy']:>8.4f} "
