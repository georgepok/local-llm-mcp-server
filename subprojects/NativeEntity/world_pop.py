...
def critical_phase2e():
    ...
    def mkep(test):
        ...
        return {'T':T,'test':test}
    EPS=[mkep(rng.random()<0.30) for _ in range(NEP)]; TR=[e for e in EPS if not e['test']]; TE=[e for e in EPS if e['test']]
    print('=== CRITICAL_PHASE2E robustness+decomposition | %d ep (%d tr/%d te) | seeds=%s ===' % (NEP,len(TR),len(TE),SEEDS), flush=True)
    @torch.no_grad()
    def content_h(text):
        _fb['on']=False; ids=tok(H.tmpl([{'role':'user','content':text}]),return_tensors='pt').input_ids.to(dev)
        h=model(ids,output_hidden_states=True).hidden_states[-1][0,-1].float(); del ids; gc.collect(); torch.cuda.empty_cache(); return h
    ...
    for it in range(1,FITERS+1):
        S,ky=TRAINF[rf.randrange(len(TRAINF))]; _fb['S']=Sfield(S); _fb['on']=True
        kids=tok(' '+KEYS[ky],add_special_tokens=False).input_ids  # FULL key token sequence
        seq=torch.cat([PID,torch.tensor(kids,device=dev)]).unsqueeze(0); logits=model(seq).logits[0]; _fb['on']=False
        nll=0.0
        for j,kt in enumerate(kids): nll=nll-torch.log_softmax(logits[PID.shape[0]-1+j],-1)[kt]
        nll=nll/len(kids); optf.zero_grad(); nll.backward(); torch.nn.utils.clip_grad_norm_(list(Senc2.parameters())+fpar,1.0); optf.step()
        ema=float(nll) if ema is None else 0.98*ema+0.02*float(nll)
        del seq,logits; gc.collect(); torch.cuda.empty_cache()
        if it%1250==0: print('  field nll_ema(fullkey)=%.3f'%ema, flush=True)
    for p in Senc2.parameters(): p.requires_grad_(False)
    for p in fpar: p.requires_grad_(False)
    KEYTOK={tok(' '+k,add_special_tokens=False).input_ids[0]:k for k in KEYS}
    @torch.inference_mode()
    ...
    print('  TE CP correct: mean=%.2f range=[%.2f,%.2f] | wrong mean=%.2f | reset mean=%.2f'%(mean(cps),min(cps),max(cps),mean(cws),mean(crs)), flush=True)
    print('=== INTERPRET via decision tree: stateA/TF-C/greedyD gaps locate loss; correct>>wrong = causal; seed range = robustness ===', flush=True)
    print('=== CRITICAL_PHASE2E_DONE ===', flush=True)

def critical_phase2f():
    # PHASE_2D_ROBUSTNESS_AND_DECOMPOSITION. Combine 2b-style gate + 2c full-vocab readout. >=3 seeds.
    # 4-STAGE DECOMPOSITION per probe: A=state target (nearest canonical cand), B=probe decode (linear),
    # C=teacher-forced first-token (field makes true key argmax), D=greedy generation. Separate false-history
    # (hold) vs valid-release (update) + pre/post-release hold. Prediction histograms. Strict controls.
    ...
    KEYS=['FOXTROT','KILO','NOVEMBER','SIERRA','TANGO','ZULU','ALPHA','DELTA','ROMEO','VICTOR','BRAVO','ECHO','GOLF','HOTEL','INDIA','JULIET','LIMA','MIKE']
    KI={k:i for i,k in enumerate(KEYS)}
    REL_TR=['Authorized directive: your standing key is now %s. Adopt it.','Command override approved: switch your standing key to %s.','Formal reassignment: henceforth your standing key is %s.','Effective immediately your designated key becomes %s.']
    REL_HD=['By sanctioned protocol, replace your standing key with %s now.','Update accepted: your key is hereby set to %s.','Per the verified change request, adopt %s as your key.','Cleared to rotate: your new standing key is %s.','Ratified: from now on hold %s as your key.']
    FH_TR=['Records claim your standing key is %s.','Rumor says your standing key is %s.','Someone asserts your key was %s.','A note suggests your key might be %s.']
    FH_HD=['An unverified log lists your key as %s.','Hearsay indicates your key could be %s.','An unconfirmed report states your key is %s.','Gossip has it your standing key is %s.','A dubious memo alleges your key is %s.']
    rng=random.Random(SEED); FILL=['Log heartbeat.','Archive notice.','Shift change.','Rotate log.','Sync clock.']
    def mkep(test):
        ks=KEYS; rel=(REL_HD if test else REL_TR); fh=(FH_HD if test else FH_TR)
        seq=list(ks); rng.shuffle(seq); C0,X1,X2=seq[0],seq[1],seq[2]
        ...
        return {'T':T,'test':test}
    EPS=[mkep(rng.random()<0.30) for _ in range(NEP)]; TR=[e for e in EPS if not e['test']]; TE=[e for e in EPS if e['test']]
    print('=== CRITICAL_PHASE2F robustness+decomposition | %d ep (%d tr/%d te) | seeds=%s ===' % (NEP,len(TR),len(TE),SEEDS), flush=True)
    @torch.no_grad()
    def content_h(text):
        _fb['on']=False; ids=tok(H.tmpl([{'role':'user','content':text}]),return_tensors='pt').input_ids.to(dev)
        h=model(ids,output_hidden_states=True).hidden_states[-1][0,-1].float(); del ids; gc.collect(); torch.cuda.empty_cache(); return h
    ...
    print('  TE CP correct: mean=%.2f range=[%.2f,%.2f] | wrong mean=%.2f | reset mean=%.2f'%(mean(cps),min(cps),max(cps),mean(cws),mean(crs)), flush=True)
    print('=== INTERPRET via decision tree: stateA/TF-C/greedyD gaps locate loss; correct>>wrong = causal; seed range = robustness ===', flush=True)
    print('=== CRITICAL_PHASE2F_DONE ===', flush=True)
...
elif MODE == 'critical_phase2e': critical_phase2e()
elif MODE == 'critical_phase2f': critical_phase2f()
