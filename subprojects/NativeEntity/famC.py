...
# --------------------------------------------------------------------------- #
class CFG:
    SRC = 0.22; SRC_COST = 0.5; COST = 1.0; DEATH = 0.06
    MUT = 0.06; CLOSE_P = 0.9
    NPOP = 5; NGEN = 20; TICKS = 220
    BOTTLE_STEPS = 110; EST_STEPS = 80; PROP_N = 14; SEED_N = 36
    MINAB = 2; NSEED = 8; P0 = 380.0
...
def make_pool(seed):
    ...
    reg = Counter()
    for _ in range(6):
        reg[random_food(rng, 1, 3)] += rng.randint(6, 10)
    for _ in range(rng.randint(2, 4)):                    # translators are a comparable heritable species
...
# --------------------------------------------------------------------------- #
def leave_one_out(reg, seedv, K=4, rec_steps=110):
    pre = Counter(reg)
    if sum(pre.values()) < 8:
        return 0.0, {}, {}, False
    ...
    any_other = False
    for j in types:
        rseed = seedv * 1000 + dh(j) % 1000               # CLONED across arms -> matched dynamics
        # intervention arm: delete j
        arm = Counter(pre); arm.pop(j, None)
        S0 = cosim(pre, arm)
        track = []
        ...
        for (T, X, Y, sc) in track:
            if Y == j and (not sc) and X != j:            # regenerated by an OTHER fragment via translation
                ...
                R[T][j] += 1
        # matched control arm: delete an abundance-matched OTHER type, SAME cloned RNG
        others = [t for t in types if t != j]
...
def viability(reg_final, events, seedv):
    ...
    return 0.002 * events + 1.0 * retained + 0.5 * est    # throughput + retention + establishment
...
def evolve(seed, cond):
    ...
    pops = [make_certified_pool(seed * 100 + i, tag)[0] for i in range(CFG.NPOP)]
    gen0_union = set()
    for p in pops:
        gen0_union |= effective_edge_set(p, CFG.MINAB)
...
def run_positive_control():
    ...
    print(f"      delete cycle substrate 'c': G={Gc[0]:+.3f}  regen-by-others(events)={Gc[1]}  selective-G={Gc[2]:+.3f}", flush=True)
    print(f"      delete matched non-cycle 'e': G={Ge[0]:+.3f}  regen-by-others(events)={Ge[1]}  selective-G={Ge[2]:+.3f}", flush=True)
    ...
    _, _, Gn, _ = leave_one_out(pn, seedv=7, K=5, rec_steps=200)
    Gcn = Gn.get('c', (0, 0, 0))
    print(f"    WITHOUT complement (U1 only):", flush=True)
    print(f"      delete 'c': G={Gcn[0]:+.3f}  regen-by-others(events)={Gcn[1]}  (expect COLLAPSE: no U2 to reconstruct)", flush=True)
    ...
    ok = (Gc[1] > 0) and (Gc[2] > 0.0) and (Gc[1] > Ge[1]) and (Gcn[1] <= Gc[1] * 0.5)
    print(f"    POSITIVE CONTROL {'PASSES' if ok else 'FAILS'}: harness detects selective constructed-"
          f"reconstruction of 'c' AND its collapse when U2 removed.", flush=True)
    if not ok:
...
def run_smoke():
    ...
    for cond in conds:
        for s in range(CFG.NSEED):
            pops, gen0_union = evolve(s, cond)
            final_union = set()
            m2 = 0; best_m3 = 0.0
            for p in pops:
                final_union |= effective_edge_set(p, CFG.MINAB)
                if has_reconstructive_cycle(p, CFG.MINAB):
                    m2 += 1
                b, _, _, _ = leave_one_out(p, seedv=s * 17 + 3)
