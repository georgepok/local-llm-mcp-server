...
def trace_causal_chain(self, target: str, max_hops: int = 10) -> Dict[str, Any]:
    ...
    for _ in range(max_hops):
        ...
        if not preds:
            break
        # Sort by mention_count descending, then by node ID ascending for determinism
        preds.sort(key=lambda n: (-self.G.nodes[n].get("mention_count", 1), n))
        current = preds[0]
        visited.add(current)
        path.append(current)
    path.reverse()
...
def test_improvement(db_factory):
    db = db_factory()
    # Create a graph with two predecessors of equal mention_count
    db.add_fragment({"nodes": [{"id": "A"}, {"id": "B"}, {"id": "C"}, {"id": "T"}],
                     "edges": [{"src": "B", "dst": "T", "type": "causes"},
                               {"src": "C", "dst": "T", "type": "causes"}]})
    # Set equal mention_count for B and C
    db.G.nodes["B"]["mention_count"] = 5
    db.G.nodes["C"]["mention_count"] = 5
    # Trace backward from T
    result = db.trace_causal_chain("T")
    # Since B < C lexicographically, B should be chosen over C
    assert result["path"] == ["B", "T"], f"Expected path ['B', 'T'], got {result['path']}"
"""
TARGET_METHOD = "trace_causal_chain"
