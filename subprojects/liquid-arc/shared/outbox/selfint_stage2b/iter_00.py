...
def test_improvement(db_factory):
    ...
    db.add_fragment({"nodes": [{"id": "A"}, {"id": "B"}, {"id": "C"}], "edges": []})
    # Set equal mention_count for B and C
    db.G.nodes["B"]["mention_count"] = 5
    db.G.nodes["C"]["mention_count"] = 5
    # Add causal edges from both B and C to A
    db.add_fragment({"edges": [{"src": "B", "dst": "A", "type": "causes"}, {"src": "C", "dst": "A", "type": "causes"}]})
    # Trace backward from A; should pick the lexicographically smaller node (B < C)
    result = db.trace_causal_chain("A")
    assert result["path"] == ["B", "A"], f"Expected path [\"B\", \"A\"], got {result['path']}"
"""
