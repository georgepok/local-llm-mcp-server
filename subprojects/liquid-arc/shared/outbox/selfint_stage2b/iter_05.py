...
def test_improvement(db_factory):
    ...
    result = db.get_neighbors(["A", "A", "A"], hops=2, direction="forward")
    # Should only return {B, C}, not inflated by duplicates
    assert result == {"B", "C"}, f"Expected {{'B', 'C'}}, got {result}"
    # Also test with duplicate in middle of list
    result2 = db.get_neighbors(["A", "B", "A", "B"], hops=1, direction="both")
