...
def generate_cases(n_cases: int, seed: int = 42) -> List[Dict[str, Any]]:
    ...
    # Total combinations: 3 domains * 6 shapes = 18 unique shape-domain pairs
    # Distribute n_cases across these pairs
    
    total_combinations = len(domains) * len(shapes)
    cases_per_combination = max(1, n_cases // total_combinations)
    ...
    for domain in domains:
        for shape_idx in shapes:
            for _ in range(cases_per_combination):
                ...
                # Build fragment based on shape_idx
                nodes = []
                edges = []
                prefix = f"{case_id}_"
                ...
                if shape_idx == 0:
                    ...
                    for i, nid in enumerate(node_ids):
                        nodes.append({"id": nid, "type": node_types[i], "role": "root" if i == 0 else ("intermediate" if i < len(node_ids) - 1 else "terminal")})
                    for i in range(len(node_ids) - 1):
                        edges.append({"src": node_ids[i], "dst": node_ids[i+1], "type": "causes", "scope": None})
                        
                elif shape_idx == 1:
                    # Linear chain, 5 nodes, edge type 'precedes'
                    node_ids = [f"{prefix}n0", f"{prefix}n1", f"{prefix}n2", f"{prefix}n3", f"{prefix}n4"]
