...
def query_temporal(self, query_nodes, k):
    ...
    for n in candidates:
        ...
        temp_scores.append((n, delta))
    
    # If all deltas are zero, set max_delta to 1.0 to avoid division by zero
    if max_delta == 0.0:
        max_delta = 1.0
    
    # Compute temporal and causal scores for each candidate
    results = []
    undirected_G = self.G.to_undirected()
    
    for n, delta in temp_scores:
        temporal_score = 1.0 - (delta / max_delta)
        
        # Compute causal score using shortest path distance
        min_distance = float('inf')
        for q in query_nodes:
            if nx.has_path(undirected_G, n, q):
                distance = nx.shortest_path_length(undirected_G, n, q)
                min_distance = min(min_distance, distance)
        
        causal_score = 1.0 / (1.0 + min_distance) if min_distance != float('inf') else 0.0
