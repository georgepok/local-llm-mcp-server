...
def query_temporal(self, query_nodes, k):
    ...
    for n in candidates:
        ...
        temporal_scores.append((n, delta))
    
    # If all deltas are zero, set max_delta to 1.0 to avoid division by zero
    if max_delta == 0.0:
        max_delta = 1.0
    
    # Calculate causal scores using shortest path on undirected graph
    G_undirected = self.G.to_undirected()
    causal_scores = {}
    for n in candidates:
        min_distance = float('inf')
        for q in query_nodes:
            if G_undirected.has_node(q) and G_undirected.has_node(n):
                try:
        ...
        if min_distance == float('inf'):
            ...
            causal_scores[n] = 1.0 / (1.0 + min_distance)
    
    # Combine scores
    results = []
    for n, delta in temporal_scores:
