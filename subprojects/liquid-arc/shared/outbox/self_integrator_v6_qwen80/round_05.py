...
def generate_cases(n_cases: int, seed: int = 42) -> List[Dict[str, Any]]:
    ...
    total_shapes = len(shape_configs)
    base_count = max(1, n_cases // total_shapes)
    extra_cases = n_cases % total_shapes

    for i, config in enumerate(shape_configs):
        count = base_count + (1 if i < extra_cases else 0)
        for _ in range(count):
            case_id = f"case_{case_counter}"
            case_counter += 1
            shape_tag = f"{config['domain']}_shape_{config['shape_id']}"
            
            # Generate descriptive text based on domain and shape
            domain = config["domain"]
            if domain == "contract_breach":
                contract_type = random.choice(["service", "sales", "lease", "employment"])
                consequence = random.choice(["damages", "financial loss", "performance failure"])
                text = (
                    f"A plaintiff alleges that the defendant failed to perform under a {contract_type} "
                    f"contract, resulting in {consequence}. The dispute centers on whether the breach "
                    f"was material and directly caused the claimed harm."
                )
            elif domain == "tort_negligence":
                duty_type = random.choice(["duty of care", "professional obligation", "safety standard"])
                harm_type = random.choice(["personal injury", "property damage", "emotional distress"])
                text = (
                    f"A plaintiff claims the defendant breached a {duty_type}, leading to {harm_type}. "
                    f"The key issue is whether the defendant's actions were the proximate cause of the harm, "
                    f"and whether reasonable care was exercised."
                )
            else:  # ip_infringement
                ip_type = random.choice(["copyright", "patent", "trademark", "trade secret"])
                violation = random.choice(["unauthorized use", "copying", "distribution", "reverse engineering"])
                text = (
                    f"A plaintiff alleges that the defendant committed {ip_type} infringement through "
                    f"{violation} of protected intellectual property. The central question is whether "
                    f"the defendant's actions violated exclusive rights granted under applicable law."
                )

            # Build graph structure based on config
            nodes = []
            edges = []
            prefix = f"{case_id}_"
            ...
            for i in range(config["node_count"]):
                ...
                nodes.append({"id": node_id, "type": "event", "role": role})
            ...
            if config["structure"] == "linear":
                ...
                # Root (node_0) -> two intermediates (node_1, node_2) -> terminal (node_3)
                edges.append({
                    "src": f"{prefix}node_0",
                    "dst": f"{prefix}node_1",
                    "type": config["edge_type"],
                ...
                # Root (node_0) connects to all other nodes (1 to 4)
                for i in range(1, config["node_count"]):
                    edges.append({
                        "src": f"{prefix}node_0",
                        "dst": f"{prefix}node_{i}",
