...
def generate_cases(n_cases: int, seed: int = 42) -> List[Dict[str, Any]]:
    ...
    for config in shape_configs:
        ...
        for _ in range(cases_per_shape):
            ...
            edges = []
            prefix = f"{case_id}_"
            node_types = ["event", "consequence", "cause", "state", "action", "result"]
            
            # Generate nodes with unique IDs
            for i in range(n_nodes):
                # Cycle through node types to add diversity
            ...
            if structure == "linear":
                ...
                for i in range(1, n_nodes):
                    edges.append({
                        "src": f"{prefix}{node_types[(i-1) % len(node_types)]}_{i-1}",
                        "dst": f"{prefix}{node_types[i % len(node_types)]}_{i}",
                        "type": edge_type,
                        "scope": None
                    })
                ...
                edges.append({
                    "src": f"{prefix}{node_types[0]}_0",
                    "dst": f"{prefix}{node_types[1]}_1",
                    "type": edge_type,
                    "scope": None
                })
                edges.append({
                    "src": f"{prefix}{node_types[0]}_0",
                    "dst": f"{prefix}{node_types[2]}_2",
                    "type": edge_type,
                    "scope": None
                })
                ...
                edges.append({
                    "src": f"{prefix}{node_types[1]}_1",
                    "dst": f"{prefix}{node_types[3]}_3",
                    "type": edge_type,
                    "scope": None
                })
                edges.append({
                    "src": f"{prefix}{node_types[2]}_2",
                    "dst": f"{prefix}{node_types[3]}_3",
                    "type": edge_type,
                    "scope": None
                })
                ...
                root_id = f"{prefix}{node_types[0]}_0"
                for i in range(1, n_nodes):
                    leaf_id = f"{prefix}{node_types[i % len(node_types)]}_{i}"
                    edges.append({
                        "src": root_id,
                        "dst": leaf_id,
                        "type": edge_type,
