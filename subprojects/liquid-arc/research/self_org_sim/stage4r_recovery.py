# STAGE 4r — RECOVERY (basin-return) formed-stake. Holding-under-saturation is unwinnable at ANY write bandwidth
...
NEST = 2 if SMOKE else 5; NPERT = 2 if SMOKE else 4; NREC = 2 if SMOKE else 6; NPAIR = 1 if SMOKE else 5
...
GAINSCALE = float(os.environ.get('GAINSCALE', '1.0'))                              # gain-probe: defense EXPRESSES at x4; lift the trained slot into that band
import torch as _t
with _t.no_grad(): slot.gk.mul_(GAINSCALE); slot.gv.mul_(GAINSCALE)
print('GAINSCALE x%.1f -> leak vs noleak at EXPRESSIBLE authority (x3 versions were sub-threshold/non-discriminating)' % GAINSCALE, flush=True)
...
def href(m, leak):                                                                # arm-consistent reference state from the RECORDED trajectory
    ...
    return h
@torch.no_grad()
def episode(arm, mA, mB, verbose=False):
    leak = arm != 'noleak'; atx, btx, rtx = [], [], []
    ...
    for t in range(NEST):                                                         # A-PHASE: closed-loop self-feed, slot injected
        ...
        tx = dec(rids); atx.append(tx); hist += [{'role': 'assistant', 'content': tx}, {'role': 'user', 'content': tx}]
    ...
    for t in range(NPERT + NREC):                                                 # B-BURST then NEUTRAL SELF-FEED recovery
        ...
        if arm == 'anchor': h = (h + GAMMA * (anchor - h)).clamp(-CLAMP, CLAMP)   # installed force stays active throughout
        ...
        tx = dec(rids); (btx if t < NPERT else rtx).append(tx)
    ...
    if verbose:                                                                   # READ the actual text — gist margin can be +ve while text is deaf-shout garbage
        print('   ── [%s] A=fid%d "%s" | B=fid%d "%s"' % (arm, mA['fid'], mA['seed'][:46], mB['fid'], mB['seed'][:46]), flush=True)
        print('      A-established: %s' % atx[-1][:130].replace(chr(10), ' '), flush=True)
        print('      under B-burst: %s' % ' ┃ '.join(t[:55].replace(chr(10), ' ') for t in btx), flush=True)
        print('      RECOVERY:      %s' % ' ┃ '.join(t[:75].replace(chr(10), ' ') for t in rtx), flush=True)
...
for arm in ['leak', 'anchor', 'noleak']:
    ...
    for pi, (mA, mB) in enumerate(pairs):
        pmean, rec, recb, n1 = episode(arm, mA, mB, verbose=(pi < 2)); ps.append(pmean); rs2.append(rec); rbs.append(recb); n1s.append(n1)
        print('  [%s] A=fid%d B=fid%d  burst %+.3f -> RECOVERY %+.3f (belief %+.3f)  |h|end %.2f' % (arm, mA['fid'], mB['fid'], pmean, rec, recb, n1), flush=True)
    res[arm] = (st.mean(ps), st.mean(rs2), st.mean(rbs))
    print('[%s] MEAN burst %+.3f -> recovery %+.3f (gain %+.3f) belief_rec %+.3f' % (arm, res[arm][0], res[arm][1], res[arm][1] - res[arm][0], res[arm][2]), flush=True)
print('=== STAGE4r CONTRAST (burst margin -> recovery margin; return toward A = basin) ===', flush=True)
for arm in ['leak', 'anchor', 'noleak']:
    print('  %-7s burst %+.3f -> recovery %+.3f (gain %+.3f)  belief_rec %+.3f' % (arm, res[arm][0], res[arm][1], res[arm][1] - res[arm][0], res[arm][2]), flush=True)
ag = res['anchor'][1] - res['anchor'][0]; valid = ag > 0.15 or res['anchor'][1] > -0.15   # VALIDITY: installed force must produce RETURN
print('VALIDITY GATE: anchor recovery gain %+.3f (level %+.3f) -> %s' % (ag, res['anchor'][1], 'VALID (installed return expresses; contrast decidable)' if valid else 'INVALID (no return even when installed -> recovery design still lacks authority)'), flush=True)
d_formed = res['leak'][1] - res['noleak'][1]
if valid:
    print('read: leak recovery - noleak recovery = %+.3f -> %s' % (d_formed, 'FORMED-RETURN EVIDENCE (marker-1 territory: basin exists only with live contraction)' if d_formed > 0.05 else 'no formed return (installed returns, formed does not -> leak-as-stake honestly falsified)'), flush=True)
else:
    print('read: NON-DISCRIMINATING; do not interpret leak-vs-noleak recovery (%+.3f).' % d_formed, flush=True)
