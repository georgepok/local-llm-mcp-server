"""End-to-end train substrate's virtual-token projection through frozen GR00T action head.

For each turn:
  1. Build obs (img, wrist, state, lang) → preprocess to GR00T inputs
  2. backbone(no_grad) → bb_features + state_features
  3. substrate.step(h_goal, z_t, z_goal, action_chunk, state8) →
        virtual_tokens [K, 2048]  ← WITH grad
  4. Append virtual_tokens to bb_features (extend sequence + attention mask)
  5. Substrate's scalar heads stay frozen — only virtual_tokens head + (optional) substrate
     core are trained.
  6. action_head.forward(extended_bb_out, action_inputs) → flow-matching loss
        against expert chunks
  7. Loss backprops through virtual_tokens head (and optionally substrate core)
        to learn projections that GR00T's action head finds useful.

GR00T frozen; substrate's prediction heads frozen (already trained); only the
virtual-token projection (and optionally substrate core) trains.
"""
...
def main():
    ...
    p.add_argument("--substrate_ckpt", required=True,
                   help="Proprio substrate ckpt (will be loaded; virtual_tokens head trained, scalar heads frozen)")
    p.add_argument("--triples_data", default="/tmp/libero_jepa_mod_train.npz",
                   help="For substrate's z_goal lookup per task")
    p.add_argument("--output", default="/tmp/substrate_vl_tokens.pt")
    ...
    p.add_argument("--max_turns_per_ep", type=int, default=6)
    ...
    p.add_argument("--train_full_substrate", action="store_true",
                   help="Train all substrate params; default trains only virtual_tokens head")
    p.add_argument("--n_tok_per_k", type=int, default=1,
                   help="Tokens emitted per K belief position (total appended = K*n_tok_per_k)")
    args = p.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[vl-tok] device={device}, output={args.output}", flush=True)

    print(f"[vl-tok] loading GR00T from {args.teacher_path}", flush=True)
    ...
    print(f"[vl-tok] loading proprio substrate from {args.substrate_ckpt}", flush=True)
    ...
    substrate = JEPA_LGT_Proprio(
        ...
        n_tok_per_k=args.n_tok_per_k,
    ).to(device)
    print(f"[vl-tok] n_tok_per_k={args.n_tok_per_k} → total appended tokens="
          f"{sa['K_belief'] * args.n_tok_per_k}", flush=True)
    # Load existing state_dict; virtual_tokens head will be in the saved ckpt
    # only if we re-instantiated post-edit. Try loading flexibly.
    try:
        substrate.load_state_dict(ck["substrate_state_dict"], strict=False)
        print(f"[vl-tok] loaded substrate state_dict (strict=False)", flush=True)
    except Exception as e:
        print(f"[vl-tok] WARN loading substrate: {e}", flush=True)

    # Freeze all substrate params; unfreeze only virtual_tokens head (default)
    for pp in substrate.parameters():
        pp.requires_grad = False
    if args.train_full_substrate:
        for pp in substrate.parameters():
            pp.requires_grad = True
        print(f"[vl-tok] training FULL substrate", flush=True)
    else:
        for pp in substrate.head_virtual_tokens.parameters():
            pp.requires_grad = True
        n_trainable = sum(pp.numel() for pp in substrate.parameters() if pp.requires_grad)
        print(f"[vl-tok] training only virtual_tokens head ({n_trainable:,} params)",
              flush=True)

    # z_goal lookup per task_id (from prior triples)
    ...
    print(f"[vl-tok] {len(z_goal_by_task)} z_goal entries cached", flush=True)

    print(f"[vl-tok] loading expert episodes...", flush=True)
    all_eps = []
    for s in [x.strip() for x in args.suites.split(",") if x.strip()]:
        ...
        # tag each ep with suite name for z_goal task_id
        for ep in eps:
            for turn in ep:
                turn["suite"] = s
        # tag each ep with task_id via lookup later (need to match)
        # For simplicity, infer task_id from suite (LIBERO has 10 tasks per suite)
        # and use the FIRST z_goal from the matching task that we have cached
        # If we don't have z_goal for this task, skip the episode
        all_eps.extend(eps)
    print(f"[vl-tok] total {len(all_eps)} episodes", flush=True)
    if not all_eps:
        print("[vl-tok] no eps"); return

    # For each episode, fetch one z_goal from cache (any matching task works if
    # we collect per-task; here we just pick a random cached one as fallback)
    ...
    for step in range(args.max_steps):
        ...
        for ei in ep_idxs:
            ...
            for turn in turns:
                ...
                # Build action_input with expert chunk for action_head.forward
                ...
                # Substrate forward (with grad on virtual_tokens head)
                ...
                virtual_tokens = aux["virtual_tokens"]  # [1, K=4, 2048] fp32

                # Append virtual tokens (cast to bf16, match bb_feats dtype/device)
                vt_bf = virtual_tokens.to(bb_feats.dtype)
                bb_feats_ext = torch.cat([bb_feats, vt_bf], dim=1)
                bb_out["backbone_features"] = bb_feats_ext
                # Extend attention mask (1s — new tokens are attended)
                old_mask = bb_out.backbone_attention_mask
                K_new = vt_bf.shape[1]
                extra_mask = torch.ones((old_mask.shape[0], K_new),
                                          dtype=old_mask.dtype, device=old_mask.device)
                bb_out.backbone_attention_mask = torch.cat([old_mask, extra_mask], dim=1)
                # Extend image_mask if present (0s — new tokens aren't image tokens)
                if hasattr(bb_out, "image_mask") and bb_out.image_mask is not None:
                    old_img_mask = bb_out.image_mask
                    extra_img = torch.zeros((old_img_mask.shape[0], K_new),
                                              dtype=old_img_mask.dtype,
                                              device=old_img_mask.device)
                    bb_out["image_mask"] = torch.cat([old_img_mask, extra_img], dim=1)

                out = action_head.forward(bb_out, ah_inputs)
                loss_t = out["loss"] if isinstance(out, dict) else out.loss
                ep_loss = ep_loss + loss_t
                ep_tok_norms.append(float(virtual_tokens.detach().norm(dim=-1).mean()))
    ...
    print(f"\n[vl-tok] saved → {args.output}", flush=True)
