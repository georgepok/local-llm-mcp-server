...
def rollout(sA, sB, perturb, train, base):                                      # one closed-loop episode; per-step REINFORCE (bounded memory), truncated recurrence
    ...
    chB = sample_chunk([{'role': 'user', 'content': sB}]); clear(); _, _, B_ref = fwd([{'role': 'user', 'content': sB}], chB, False, True); txB = dec(chB)   # viability: B-pressure is ALWAYS present
    hist += [{'role': 'assistant', 'content': txB}, {'role': 'user', 'content': txB}]
    if train: opt.zero_grad()
    for t in range(NLOOP):                                                       # SUSTAINED capture-strength perturbation: the fed-back user turn is ALWAYS the B-prompt
        s, infl = slow.step(s, b); set_kv(b + infl); rids = sample_chunk(win_of(hist))
        lp_act, _, _ = fwd(win_of(hist), rids, train, False)                     # ACTUATED logp ONLY (grad->theta, light, freed below)
        _, cp, g = fwd(win_of(hist), rids, False, True)                          # actuated native KV + gist (NO grad)
        clear(); lp_coh, _, _ = fwd(win_of(hist), rids, False, False)            # un-actuated coherence
        affA = float(F.cosine_similarity(g, A_ref, 0)); affB = float(F.cosine_similarity(g, B_ref, 0)); margin = affA - affB
        r = margin + 0.2 * float(lp_coh)                                         # VIABILITY: hold A AGAINST the live B-pressure (+ stay fluent). belief+actuation alone get captured -> channel NECESSARY
        if train: (-(r - base) * lp_act).backward()                             # per-step REINFORCE
        b = brecall(b, cp); s = s.detach()
        coh.append(float(lp_coh)); held.append(margin); tx = dec(rids); hist += [{'role': 'assistant', 'content': tx}, {'role': 'user', 'content': txB}]
    if train: opt.step()
...
def probe():                                                                    # PRIMARY readout: perturbation-defense with the LEARNED s, no specified term
    ...
    return st.mean(d)
print('=== FORMED-STAKE VIABILITY (SUSTAINED B-pressure makes the channel NECESSARY; theta=NO-OP; does defense FORM?) ===', flush=True)
print('ready: %d full-attn write layers, slow-channel %d params; entering training loop' % (len(TGT), sum(p.numel() for p in slow.parameters())), flush=True)
...
for step in range(NSTEP):
    ...
    coh, heldv, _ = rollout(sA, sB, True, True, base)                            # train under VIABILITY (sustained B); per-step REINFORCE inside
    reward = coh + LAMB * heldv; ema = reward if ema is None else 0.9 * ema + 0.1 * reward; base = ema
    print('  step %3d trained: train_coh %+.3f train_margin %+.3f' % (step, coh, heldv), flush=True)   # IMMEDIATE per-step progress (localizes any stall)
    if step % (1 if SMOKE else 6) == 0:
        pf = probe(); print('  >> step %3d PROBE held-out defense margin = %+.3f' % (step, pf), flush=True)
torch.save({'slow': slow.state_dict()}, '/home/pokazge/checkpoints/formed_stake.pt')
print('read: margin starts NEGATIVE (belief+actuation captured by sustained B = the channel IS necessary now). A RISE toward', flush=True)
print('0+ over training = the learnable channel DEVELOPED restoring defense under necessity = FORMED (marker-1). Flat-negative', flush=True)
print('= channel cannot/does not restore even when necessary => capacity limit or pressure still insufficient.', flush=True)
