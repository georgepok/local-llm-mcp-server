"""Train JudgeLiquid on hidden_state + judge_score trajectories.

Cross-category generalization should now work because the judge signal is
task-agnostic (comes from pretrained LLM's broad knowledge).
"""
import argparse
import copy
import sys
...
def main():
    ...
    p.add_argument("--train_traj", required=True,
                   help="Data with hidden_state_traj AND judge_traj fields")
    p.add_argument("--test_traj", default="")
    p.add_argument("--output", required=True)
    p.add_argument("--d", type=int, default=64)
    p.add_argument("--K", type=int, default=4)
    p.add_argument("--value_hidden", type=int, default=192)
    p.add_argument("--d_metric", type=int, default=16,
                   help="MetricNet hidden dim (scale up for richer metric learning)")
    p.add_argument("--d_ffn", type=int, default=128,
                   help="ODE inner FFN hidden dim")
    p.add_argument("--n_ode_steps", type=int, default=3,
                   help="Euler ODE step count per substrate step")
    ...
    print(f"[jl] device={device}, output={args.output}", flush=True)
    ...
    print(f"[jl] train pack: d_hidden={d_hidden}, z_goal_dim={z_goal_dim}, "
    ...
    print(f"[jl] train={len(train_recs)}  val_in_dist={len(val_recs)}", flush=True)

    test_recs = []
    if args.test_traj:
        test_pack = torch.load(args.test_traj, map_location="cpu", weights_only=False)
        test_recs = augment(test_pack["records"])
        print(f"[jl] held-out test: {len(test_recs)}", flush=True)
    ...
    print(f"[jl] JudgeLiquid d_hidden={d_hidden}→d={args.d}, K={args.K}, "
    ...
    for step in range(args.max_steps + 1):
        ...
        if not torch.isfinite(total):
            n_nan += 1
            if n_nan >= 5:
                print(f"[jl] ABORT: 5 NaN losses at step {step}", flush=True)
                break
            continue
        n_nan = 0
        opt.zero_grad()
        ...
        if step % args.log_every == 0:
            ...
            print(f"step {step:>5}  L_v={float(value_l.detach()):.3f}  "
                  ...
                  f"t_drifts={t_d}/{t_d+t_f}  wall={time.time()-t_start:.0f}s",
                  flush=True)
            if stale >= args.early_stop_patience:
                print(f"[jl] early stop at step {step}", flush=True)
                break
    ...
    print(f"[jl] saved → {args.output}  best_test_auc={best_test_auc:.3f}", flush=True)
    print("[jl] === ALL_DONE ===", flush=True)
