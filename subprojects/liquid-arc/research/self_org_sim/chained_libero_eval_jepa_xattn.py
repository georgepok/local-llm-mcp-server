"""Chained-LIBERO eval with X-attn JEPA-LGT (per-token tangent override).

Per turn:
1. Get FULL bb_features [seq, 2048] via op=get_full_bb_features
2. Substrate.step(h, bb_t, z_goal, last_chunk) → per-token tangent [seq, 2048]
3. Send tangent as per_token_override → GR00T's action head decodes from
   bb_features + tangent at every token (no uniform broadcast).
"""
...
class GrootJepaXAttnClient:
    def __init__(self, port, substrate_ckpt, suite_name, device=None,
        ...
        horizon = ck["horizon"]
        self.substrate = JEPA_LGT_XAttn(
            z_vl_dim=z_dim, action_dim=action_dim, horizon=horizon,
            d=sa["d_substrate"], K=sa["K_belief"], n_heads=sa["n_heads"],
        ...
        self.total_tn_per_token = 0.0
        self.total_tn_total = 0.0
        self.n_chunks = 0
        print(f"[xattn-eval] loaded {substrate_ckpt}, d={sa['d_substrate']}, "
              f"K={sa['K_belief']}, heads={sa['n_heads']}, "
    ...
    def query(self, obs_dict):
        # 1. FULL bb_features from GR00T (per-token, not pooled)
        self.sock.send(pickle.dumps({"op": "get_full_bb_features", "obs": obs_dict}))
        resp1 = pickle.loads(self.sock.recv())
        bb_t = resp1["bb_full"].astype(np.float32)  # [seq, 2048]

        # 2. substrate per-token tangent
        with torch.no_grad():
            bb_t_torch = torch.from_numpy(bb_t).to(self.device).unsqueeze(0)
            ...
            h_new, bb_pred, tangent, info = self.substrate.step(
                self.h_goal, bb_t_torch, z_goal_torch, lc)
            self.h_goal = h_new
            tangent_np = (tangent[0].cpu().numpy() * self.lookahead_lambda
                          ).astype(np.float32)  # [seq, 2048]
            self.total_tn_per_token += float(info["tangent_norm_per_token"])
            self.total_tn_total += float(info["tangent_total_norm"])

        # 3. GR00T action with PER-TOKEN tangent override
        self.sock.send(pickle.dumps({"op": "get_action_with_per_token_override",
...
def main():
    ...
    p.add_argument("--out_json", default="/tmp/chained_libero_jepa_xattn.json")
    args = p.parse_args()

    client = GrootJepaXAttnClient(
    ...
    print(f"[xattn-eval] suite={args.task_suite} chains={len(chains)} "
