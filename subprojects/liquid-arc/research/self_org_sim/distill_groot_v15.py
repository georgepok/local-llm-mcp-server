"""v15 trainer: Liquid as decoder over GR00T features. No DINOv2.

Inputs (all from GR00T's training-time captures):
  z_vl_bank: [B, K_bank=4, 1024]    — GR00T's depth-bank features
  z_state:   [B, 1536]               — GR00T's state encoding
  robot_state: [B, 8]                — proprioception
  teacher_chunk: [B, 16, 7]           — supervision target

Liquid encoder (V15Encoder): substrate over per-modality positions, no image processing.
"""
...
class V15Policy(nn.Module):
    def __init__(self, config, action_horizon=16, action_dim=7, state_dim=8,
                 head_d=256, head_layers=4, head_heads=4,
                 z_vl_dim=1024, z_state_dim=1536, K_bank=4, K_latent=2):
        super().__init__()
        self.action_horizon = action_horizon
        self.action_dim = action_dim
        self.encoder = V15Encoder(
            config=config, state_dim=state_dim,
            z_vl_dim=z_vl_dim, z_state_dim=z_state_dim,
            K_bank=K_bank, K_latent=K_latent,
        )
        ...
        self.gripper_head = nn.Sequential(
            nn.Linear(self.encoder.d_out, 128), nn.SiLU(),
            nn.Linear(128, action_horizon),
        )

    def encode(self, z_bank, z_state, state, n_ode_steps_override=None, ablate_substrate=False):
...
def collect_aux_losses(out, config):
    losses = {}
    g = out["g"]; h_final = out["h_final"]
    tau = out["tau"]; t_diff_param = out["t_diff_param"]
...
def main():
    ...
    p.add_argument("--output_dir", default="/tmp/distill_v15", type=str)
    ...
    p.add_argument("--z_groot_drop_prob", type=float, default=0.2,
                   help="Stage-1 pressure: dropout z_bank+z_state to zero with this prob")
    p.add_argument("--resume", default="", type=str)
    args = p.parse_args()

    out_dir = Path(args.output_dir); out_dir.mkdir(parents=True, exist_ok=True)
    print(f"[v15] output: {out_dir}")
    ...
    print(f"[v15] config: d={config.d_model}, halting={config.halting_enabled}")
    ...
    for dd, ld in zip(decoded_dirs, labels_dirs):
        # return_z_groot="z_state" → loads z_state.dat into "z_groot" slot
        # use_query_bank=True → loads z_vl_bank.dat into "z_bank" slot
        # Together: per __getitem__ returns (img, wrist, state, chunk, ti, z_state, z_bank, delta_bank)
        ...
        datasets.append(ds)
    print(f"[v15] z_vl_dim={z_vl_dim}, z_state_dim={z_state_dim}, drop_prob={args.z_groot_drop_prob}")
    train_ds = ConcatDataset(datasets) if len(datasets) > 1 else datasets[0]
    print(f"[v15] samples: {len(train_ds)}")
    ...
    print(f"[v15] params: {n_total:,} total")

    optimizer = build_optimizer(model, config, args)
    print(f"[v15] geo LR: {args.lr * config.structural_lr_ratio:.2e}")

    step = 0
    log_records = []
    if args.resume:
        print(f"[v15] resuming from {args.resume}")
    ...
    while step < args.max_steps:
        ...
        # 8-tuple: (img, wrist, state, chunk, ti, z_state, z_bank, delta_bank)
    ...
    print(f"[v15] training complete: {step} steps, {time.time() - t_start:.1f}s")
