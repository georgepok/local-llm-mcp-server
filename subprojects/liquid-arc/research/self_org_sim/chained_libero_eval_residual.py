"""Chained-LIBERO eval — substrate produces BROADCAST RESIDUAL added to GR00T's bb_features.

Per turn:
  1. Get z_t + plain pass info from server
  2. substrate.step(...) → residual [2048]
  3. Re-query via get_action_with_zvl_override op (residual added to bb_features broadcast)
  4. Execute the residual-augmented chunk

Substrate's residual head trained via BC through frozen GR00T action head; residual
forces action head's cross-attention to use modified bb_features (vs vl-token append
which the action head can attentionally ignore).
"""
...
class ResidualClient:
    def __init__(self, port, substrate_ckpt, suite_name, device=None,
        ...
        print(f"[res-eval] substrate {substrate_ckpt}, K={self.K}, "
              f"res_scale={res_scale}", flush=True)
    ...
    def query(self, obs_dict, state8_now):
        # 1. plain pass to get z_t + z_lang + bb_full + chunk_groot
        ...
        z_t = resp["z_vl"]
        z_lang = resp.get("z_lang", z_t)  # JEPA-VL channel; falls back to z_vl
        bb_full = resp.get("bb_full", None)  # [seq, 2048] for cross-attn substrate

        # 2. substrate produces residual (per-token if bb_full available, else broadcast)
        with torch.no_grad():
            ...
            if bb_full is not None:
                bb_feats_t = torch.from_numpy(np.asarray(bb_full, dtype=np.float32)).to(
                    self.device).unsqueeze(0)
            ...
            self.h_goal, _, aux, _ = self.substrate.step(
                ...
                z_lang_t=z_lang_t, bb_features=bb_feats_t)
            per_tok = aux.get("per_token_residual", None)
            if per_tok is not None:
                # Keep batch dim → 3D [1, seq_len, 2048] so server handler treats as per-token
                # (handler reads dim()==3 as per-token, dim()==2 as broadcast [B,2048]).
                residual = per_tok.cpu().numpy().astype(np.float32) * self.res_scale
                use_per_token = True
            else:
                residual = aux["residual"][0].cpu().numpy().astype(np.float32) * self.res_scale
                use_per_token = False
            # Hybrid noise mix: blend substrate residual with isotropic Gaussian
            # rescaled to TOTAL L2 norm = noise_total_norm.
            nm = getattr(self, "noise_mix", 0.0)
            if nm > 0.0:
                rng = getattr(self, "_rng", None) or np.random.default_rng()
                raw_noise = rng.standard_normal(residual.shape).astype(np.float32)
                raw_norm = float(np.linalg.norm(raw_noise) + 1e-8)
                noise = raw_noise * (self.noise_total_norm / raw_norm)
                residual = (1.0 - nm) * residual + nm * noise

        # 3. re-query with residual added — per-token if cross-attn, else broadcast
        op = "get_action_with_per_token_override" if use_per_token \
              else "get_action_with_zvl_override"
        self.sock.send(pickle.dumps({
            "op": op, "obs": obs_dict, "zvl_residual": residual,
        }))
        resp2 = pickle.loads(self.sock.recv())
        chunk_aug = np.asarray(resp2["chunk"], dtype=np.float32)
        # Log avg per-token norm (for per-token 3D shape) or full norm (broadcast 1D)
        if use_per_token:
            # residual shape [1, seq, 2048] → norm per token
            self.total_res_norm += float(np.linalg.norm(residual[0], axis=-1).mean())
        else:
            self.total_res_norm += float(np.linalg.norm(residual))
        self.n_chunks += 1
        return chunk_aug
...
def main():
    ...
    p.add_argument("--res_scale", type=float, default=1.0,
                   help="Multiplier on residual at inference (1.0 = train-time scale)")
    p.add_argument("--noise_mix", type=float, default=0.0,
                   help="Hybrid noise: final_residual = (1-m)*substrate + m*N(0, sigma=tok_norm)")
    p.add_argument("--noise_total_norm", type=float, default=10.0,
                   help="Target TOTAL L2 norm of noise component (variant #9 used norm~7)")
    p.add_argument("--out_json", default="/tmp/chained_libero_residual.json")
    args = p.parse_args()

    client = ResidualClient(args.groot_port, args.substrate_ckpt, args.task_suite,
                              res_scale=args.res_scale)
    client.noise_mix = args.noise_mix
    client.noise_total_norm = args.noise_total_norm
    client._rng = np.random.default_rng(args.seed)
    print(f"[res-eval] noise_mix={args.noise_mix} "
          f"noise_total_norm={args.noise_total_norm}", flush=True)
    ...
    print(f"[res-eval] suite={args.task_suite} chains={len(chains)} "
