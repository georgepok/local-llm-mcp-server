# STAGE 2c — closed-loop KL distillation on the STAGE-1c CANONICAL LTC-BANK belief (W-free contraction; tau spectrum
...
print('STAGE2c | trajs=%d train=%d test=%d cats=%d held-out=%s | belief d=%d -> slot M=%d x layers %s' % (len(data), len(tr), len(te), len(fids), sorted(hold), D, MINJ, TGT), flush=True)
...
torch.save({'slot': slot.state_dict(), 'bel': bel.state_dict(), 'rq': ck1['rq'], 'Rp': Rp, 'tgt': TGT}, '/home/pokazge/checkpoints/stage2c_distill.pt')
