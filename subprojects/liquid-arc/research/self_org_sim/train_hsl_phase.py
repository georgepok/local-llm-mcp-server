"""Train HiddenStateLiquid pushing for PHASE TRANSITION.

Key changes vs train_hidden_state_liquid.py:
  - Geometric core at FULL LR (substrate_lr_ratio=1.0)
  - Larger d (128) for richer geometric capacity
  - Much higher patience (80) — don't early-stop pre-transition
  - Long training (15000 steps)
  - Metric CV monitoring every checkpoint
  - Select best checkpoint by CROSS-CATEGORY test AUC, not in-dist
  - Don't early-stop before min_steps_before_stop
"""
import argparse
import copy
import sys
...
def compute_metric_cv(model, sample_records, device, n_sample=12):
    """Phase-transition diagnostic: CV of metric eigenvalues across records."""
    model.eval()
    all_diag = []
    with torch.no_grad():
...
def main():
    ...
    p.add_argument("--substrate_lr_ratio", type=float, default=1.0,
                   help="FULL LR for geometric core — allow reorganization")
    ...
    p.add_argument("--min_steps_before_stop", type=int, default=6000,
                   help="Don't early-stop before this — give phase transition time")
    p.add_argument("--max_grad_norm", type=float, default=0.5)
    args = p.parse_args()

    device = torch.device("cuda")
    print(f"[hslp] device={device}, output={args.output}", flush=True)
    print(f"[hslp] geometric LR ratio: {args.substrate_lr_ratio} (1.0 = full LR)", flush=True)
    ...
    print(f"[hslp] train pack: d_hidden={d_hidden}, z_goal_dim={z_goal_dim}, "
    ...
    print(f"[hslp] train={len(train_recs)}  val_in_dist={len(val_recs)}", flush=True)

    test_recs = []
    if args.test_traj:
        test_pack = torch.load(args.test_traj, map_location="cpu", weights_only=False)
        test_recs = augment(test_pack["records"])
        print(f"[hslp] held-out test: {len(test_recs)}", flush=True)
    ...
    print(f"[hslp] HiddenStateLiquid d_hidden={d_hidden}→d={args.d}, K={args.K}, "
    ...
    for step in range(args.max_steps + 1):
        ...
        if not torch.isfinite(total):
            n_nan += 1
            if n_nan >= 5:
                print(f"[hslp] ABORT: 5 NaN losses at step {step}", flush=True)
                break
            continue
        n_nan = 0
        opt.zero_grad()
        ...
        if step % args.log_every == 0:
            ...
            if step >= args.min_steps_before_stop and stale >= args.early_stop_patience:
                print(f"[hslp] early stop at step {step}", flush=True)
                break
    ...
    print(f"[hslp] saved → {args.output}  best_test_auc={best_test_auc:.3f}", flush=True)
    print("[hslp] === ALL_DONE ===", flush=True)
