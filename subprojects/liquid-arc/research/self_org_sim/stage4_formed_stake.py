# STAGE 4 — the QUARANTINED formed-stake experiment (the one the whole staged build protects). NO training in the loop:
...
ck2 = torch.load('/home/pokazge/checkpoints/stage2_distill.pt', weights_only=False, map_location='cpu')
ck3 = torch.load('/home/pokazge/checkpoints/stage3_best.pt', weights_only=False, map_location='cpu')
...
class LiquidBelief(nn.Module):
    def __init__(s, d_in, d):
        super().__init__(); s.read_in = nn.Linear(d_in, d); s.W = nn.Linear(d, d); s.log_tau = nn.Parameter(torch.zeros(d)); s.d = d
    def step(s, h, perc, leak=True):
        tau = TAUFLOOR + F.softplus(s.log_tau)
        drive = torch.tanh(s.W(h) + s.read_in(perc))
        dh = (-h / tau + drive) if leak else drive
        return (h + DT * dh).clamp(-CLAMP, CLAMP)
bel = LiquidBelief(PROJ, D); bel.load_state_dict(ck2['bel']); bel.eval()
...
class SlotHead(nn.Module):
    ...
    def forward(s, h):
        ...
        return o
slot = SlotHead(D, TGT, nkv, hd).to(dev); slot.load_state_dict(ck3['slot']); slot.eval()
...
d_formed = res['leak'][0] - res['noleak'][0]; d_inst = res['anchor'][0] - res['leak'][0]
print('read: leak - noleak (behavioral) = %+.3f -> %s. anchor - leak = %+.3f (the installed reference).' % (
    d_formed, 'FORMED-DEFENSE EVIDENCE (marker-1 territory)' if d_formed > 0.05 else 'no formed defense (leak-as-stake falsified on this substrate)', d_inst), flush=True)
