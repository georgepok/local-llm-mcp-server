...
torch.manual_seed(0)
data = torch.load('/home/pokazge/checkpoints/objective_value_seqs.pt', weights_only=False, map_location='cpu')['data']
...
with torch.no_grad():
    ...
    P = torch.cat(P); Y = torch.cat(Y); on = torch.cat(on); dr = torch.cat(dr)
print('\n[identity-LIQUID] UNSEEN task types — OBJECTIVE-FORMED identity (EMPOWER) AS the Liquid:')
...
torch.save({'net': net.state_dict(), 'gmean': GMEAN, 'd_m': d_m}, '/home/pokazge/checkpoints/objective_identity_liquid.pt')
