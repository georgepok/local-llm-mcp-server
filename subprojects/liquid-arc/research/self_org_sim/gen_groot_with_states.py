"""GR00T-in-sim with internal-state capture (System 1 / System 2 architecture).

Same as groot_sim_collect.py but uses op="get_action_with_state", saving GR00T's
multimodal latent representations alongside each (obs, action_chunk) pair:
  z_vl     [vl_dim]    — post-Qwen3-VL backbone (image + language + intent)
  z_state  [state_dim] — proprioceptive features (post-state-encoder)
  z_motor  [7]         — final converged motor velocity (most compressed plan)

These are the "internal states" GR00T uses to reason. The Liquid student will
condition on them as System-1 reflexomotor input from a System-2 reasoner —
not on language labels (which violates the no-text rule for Liquid).
...
already running in main venv on the same port.
  ...
  python gen_groot_with_states.py --task_suite libero_10 --rollouts_per_task 10 \\
    --max_steps 720 --exec_horizon 8 --port 5555 \\
    --out_dir /home/pokazge/datasets/groot-sim-states-v1
"""
...
def main():
    ...
    z_vl_dim = None
    z_state_dim = None
    print(f"Phase 1: GR00T-in-sim collection with state capture "
    ...
    assert z_vl_dim is not None and z_state_dim is not None, \
        "no samples collected — z dims never initialized"
    ...
    summary = {
        ...
        "size_gb": (
            total_n * 256 * 256 * 3 * 2  # imgs+wrists uint8
            + total_n * 16 * 7 * 4       # chunks
            + total_n * z_vl_dim * 4
    ...
    print(f"z_vl={z_vl_dim}d, z_state={z_state_dim}d, z_motor=7d")
