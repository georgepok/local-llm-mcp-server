"""Chained-LIBERO eval with scalar substrate as STUCK-DETECTOR gate.

Mechanism:
  - Substrate predicts goal_distance at every turn (R²=0.34 on held-out per
    project_chained_libero_substrate.md scalar pivot).
  - Track moving average of predicted distance over `window` turns.
  - If recent-window-avg > older-window-avg (distance INCREASING) → stuck;
    trigger early retract within the sub-task (open + lift open-loop).
  - Otherwise pass through baseline GR00T behavior.

This tests whether the substrate's goal_distance prediction (modest but real
+0.34 R² on held-out) translates to a behavioral improvement when used as a
gate signal rather than a direct residual injector.

Compares against:
  - baseline (no substrate, no gate)
  - constant retract between sub-tasks (proven +9.2pp baseline)
"""
...
class GrootScalarGateClient:
    def __init__(self, port, substrate_ckpt, suite_name, device=None,
                 stuck_window=4, stuck_threshold=0.0,
                 use_gate=True):
        """
        stuck_window: turns to average over for stuck-detection.
        stuck_threshold: min delta (older_avg - recent_avg) BELOW which counts as
                          stuck. 0 = recent_avg >= older_avg → stuck.
        use_gate: if False, behaves as baseline (no early retract); for control.
        """
        if device is None:
        ...
        horizon = ck["horizon"]
        self.substrate = JEPA_LGT_Scalar(
        ...
        self.z_dim = z_dim
        # De-normalization stats (saved with ckpt)
        self.steps_mean = ck.get("steps_mean", 0.0)
        self.steps_std = ck.get("steps_std", 1.0)
        self.dist_mean = ck.get("dist_mean", 0.0)
        self.dist_std = ck.get("dist_std", 1.0)
        self.h_goal = None
        self.z_goal_cache = {}
        self.current_z_goal = None
        self.dist_history = []
        self.stuck_count = 0
        self.n_stuck_triggers = 0
        self.n_chunks = 0
        print(f"[gate-eval] loaded {substrate_ckpt}, d={sa['d_substrate']}, "
              f"K={sa['K_belief']}, window={stuck_window}, use_gate={use_gate}",
              flush=True)
        print(f"[gate-eval] steps_mean={self.steps_mean:.2f} "
              f"dist_mean={self.dist_mean:.2f}", flush=True)
    ...
    def query(self, obs_dict):
        """Get baseline chunk + substrate's distance prediction.
        Returns (chunk, is_stuck) — caller can decide what to do on stuck.
        """
        # Always get baseline action from GR00T
        ...
        # Substrate's distance prediction
        with torch.no_grad():
            ...
            h_new, p_steps, aux, _ = self.substrate.step(
                self.h_goal, z_t_torch, z_goal_torch, chunk_t)
            self.h_goal = h_new
            # Un-normalize distance prediction
            p_dist = float(aux["pred_goaldist"]) * self.dist_std + self.dist_mean

        # Track distance history; detect stuck
        self.dist_history.append(p_dist)
        is_stuck = False
        if self.use_gate and len(self.dist_history) >= 2 * self.stuck_window:
            recent = np.mean(self.dist_history[-self.stuck_window:])
            older = np.mean(self.dist_history[-2 * self.stuck_window:-self.stuck_window])
            if (recent - older) > self.stuck_threshold:
                # Distance has stopped decreasing (or increasing) → stuck
                is_stuck = True
                self.stuck_count += 1

        self.n_chunks += 1
        return chunk, is_stuck, p_dist
...
def step_subtask(env, sub_task, obs, args, client, max_steps=None):
    ...
    while sub_steps < cap:
        if chunk is None or chunk_idx >= args.exec_horizon or chunk_idx >= len(chunk):
            ...
            chunk, is_stuck, p_dist = client.query(groot_obs)
            chunk_idx = 0
            n_calls += 1
            calls_since_last_trigger += 1
            # Conservative trigger: respect max_triggers AND cooldown AND min_calls
            should_trigger = (
                is_stuck
...
def main():
    ...
    p.add_argument("--retract_steps", type=int, default=30,
                   help="Between-sub-task retract (proven +9.2pp baseline).")
    p.add_argument("--retract_grip_thr", type=float, default=0.030)
    p.add_argument("--shared_budget", type=int, default=0)
    # Stuck-detection gate params
    p.add_argument("--use_gate", action="store_true",
                   help="If unset, runs baseline (no in-sub-task retract).")
    p.add_argument("--stuck_window", type=int, default=4)
    p.add_argument("--stuck_threshold", type=float, default=0.0,
                   help="recent_avg - older_avg > threshold → stuck.")
    p.add_argument("--stuck_min_calls", type=int, default=3,
                   help="Don't trigger gate before this many GR00T calls.")
    p.add_argument("--stuck_retract_steps", type=int, default=20,
                   help="Steps of open+lift when stuck-trigger fires.")
    p.add_argument("--max_triggers_per_subtask", type=int, default=2,
                   help="Cap on stuck-trigger firings per sub-task.")
    p.add_argument("--trigger_cooldown_calls", type=int, default=8,
                   help="GR00T calls required between stuck triggers.")
    p.add_argument("--out_json", default="/tmp/chained_libero_scalar_gate.json")
    args = p.parse_args()

    client = GrootScalarGateClient(
    ...
    print(f"[gate-eval] suite={args.task_suite} chains={len(chains)} "
          f"chain_len={args.chain_length} use_gate={args.use_gate} "
          f"shared_budget={args.shared_budget}", flush=True)
    ...
    summary["total_stuck_triggers"] = total_stuck_triggers
    print(f"\n{'='*60}", flush=True)
    print(f"OVERALL: {total_succ}/{total_attempts} sub-tasks = "
          f"{100*summary['completion_rate']:.0f}%  "
          f"stuck_triggers={total_stuck_triggers}", flush=True)
