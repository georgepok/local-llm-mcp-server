"""Chained-LIBERO eval — substrate features enter GR00T's input via language prefix.

Substrate's scalar predictions (p_dist, p_gripper, p_state_delta) are translated
into natural-language descriptors and PREPENDED to the task description. GR00T's
Qwen3-VL backbone re-encodes; action head sees vl_embeds that reflect the
augmented language.

No GR00T modification, no new training, no chunk-level intervention.
Substrate's information enters perception via the modality GR00T already handles.

If GR00T's language model can interpret "[state: close to goal, gripper stable,
robot stationary] put the bowl on the plate" meaningfully, the action chunk
should differ from the un-augmented chunk in ways that reflect the state info.
"""
...
def descriptors_from_substrate(p_dist, p_gripper, p_state_delta,
                                  dist_close_thr=20.0, dist_far_thr=25.0,
                                  gripper_thr=0.5, motion_thr=0.5):
    if p_dist < dist_close_thr:
        dist_d = "close to goal"
    elif p_dist < dist_far_thr:
        dist_d = "approaching goal"
    else:
        dist_d = "far from goal"
    gripper_d = ("gripper changing state" if p_gripper > gripper_thr
                  else "gripper stable")
    motion_d = ("robot moving" if p_state_delta > motion_thr
                  else "robot stationary")
    return f"[state: {dist_d}, {gripper_d}, {motion_d}]"
...
class LangPrefixClient:
    def __init__(self, port, substrate_ckpt, suite_name, device=None,
        ...
        self.n_chunks = 0
        self.descriptor_log = []
        print(f"[lang-prefix] substrate {substrate_ckpt}, use_prefix={use_prefix}",
              flush=True)
    ...
    def query(self, img_256, wrist_256, state8, task_lang):
        """Two-pass: (1) plain GR00T → z_vl; (2) substrate gen descriptor; (3) re-query GR00T with augmented language."""
        # Pass 1: plain obs to get z_vl for substrate's input
        plain_obs = build_groot_obs(img_256, wrist_256, state8, task_lang)
        self.sock.send(pickle.dumps({"op": "get_action_with_state", "obs": plain_obs}))
        resp1 = pickle.loads(self.sock.recv())
        z_t = resp1["z_vl"]
        baseline_chunk = np.asarray(resp1["chunk"], dtype=np.float32)

        if not self.use_prefix:
            # Control: no language augmentation (should match baseline)
            self.n_chunks += 1
            return baseline_chunk

        # Substrate predicts → descriptor
        with torch.no_grad():
            ...
            state8_t = torch.from_numpy(state8.astype(np.float32)).to(self.device).unsqueeze(0)
            chunk_t = torch.from_numpy(baseline_chunk).to(self.device).unsqueeze(0).float()
        ...
        descriptor = descriptors_from_substrate(p_dist, p_grip, p_sd)
        self.last_descriptor = descriptor
        self.descriptor_log.append(descriptor)
        augmented_lang = f"{descriptor} {task_lang}"

        # Pass 2: re-query GR00T with augmented language
        aug_obs = build_groot_obs(img_256, wrist_256, state8, augmented_lang)
        self.sock.send(pickle.dumps({"op": "get_action_with_state", "obs": aug_obs}))
...
def step_subtask(env, sub_task, obs, args, client, max_steps=None):
    ...
    while sub_steps < cap:
        if chunk is None or chunk_idx >= args.exec_horizon or chunk_idx >= len(chunk):
            ...
            chunk = client.query(groot_img, groot_wrist, state8, sub_lang)
            chunk_idx = 0
            n_calls += 1
...
def main():
    ...
    p.add_argument("--use_prefix", action="store_true",
                   help="If unset: control run (no language augmentation).")
    p.add_argument("--out_json", default="/tmp/chained_libero_lang_prefix.json")
    args = p.parse_args()

    client = LangPrefixClient(
    ...
    print(f"[lang-prefix] suite={args.task_suite} chains={len(chains)} "
          f"L={args.chain_length} use_prefix={args.use_prefix} "
        ...
        for r in range(n_rollouts):
            ...
            for sub_idx, sub_id in enumerate(chain):
                ...
                sub_details.append({"sub_id": sub_id, "succ": succ, "steps": steps,
                                     "n_calls": n_calls,
                                     "last_descriptor": client.last_descriptor})
            ...
            last_desc = client.last_descriptor
            print(f"  r{r}: {n_complete}/{len(chain)} sub-tasks  wall={wall:.0f}s  "
                  f"steps=[{steps_str}]  last_desc=\"{last_desc}\"", flush=True)
    ...
    # Sample some descriptors for inspection
    summary["descriptor_samples"] = client.descriptor_log[:50]
    print(f"\n{'='*60}", flush=True)
