"""Goal-conditioned JEPA-LGT — z_goal enters as an explicit anchor.

Diagnosis (from random-residual ablation, 2026-05-18):
  Goal-blind JEPA-LGT (only z_t + action_chunk) tied at 81.6% vs random-noise 88.9%.
  Without explicit goal input, substrate predicts free-form trajectory, not toward target.

Fix per user spec: feed z_goal (frozen GR00T embedding of the sub-task's success state)
as a persistent anchor input. h_goal evolves toward z_goal across turns; tangent points
in the direction of progress toward z_goal.
...
class JEPA_LGT_Goal(nn.Module):
    """Goal-anchored JEPA world model in z_vl space.

    Args:
        z_vl_dim:    2048   GR00T's vl-embed dim (same for current + goal)
        action_dim:  7      raw action dim
        horizon:     16     action chunk length
        d:           64     substrate hidden dim
        K:           4      belief positions
        tangent_scale: 0.2  cap on |ẑ - z| via tanh
    """
    ...
    def __init__(self, z_vl_dim=2048, action_dim=7, horizon=16,
        ...
        # Inputs: current z, goal z, last action chunk
        ...
        # Learnable gates — goal init HIGH (anchor), action init LOW (gentle ramp)
    ...
    def step(
        ...
        """Predict z_{t+1} given current state, goal anchor, and last action."""
        ...
        # Optional: include explicit progress signal (z_goal - z_t) implicitly via e_g - e_z
        e_evidence = e_z + e_g + e_a
