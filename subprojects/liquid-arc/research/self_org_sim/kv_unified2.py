# UNIFIED NATIVE actuator, FAITHFUL (step 2, redone): keeps BOTH the Liquid AND attention-on-attention. The Liquid's
# belief QUERIES (AoA: Wq.b over Wk/Wv of the stream) over the model's NATIVE KV SEQUENCE — the per-token keys+values at
# the SAME full-attn layers it writes to (23/27/31) — then writes (k,v) back. Read KV via attention, write KV: symmetric,
# the Liquid's own attention mechanism reading/writing the model's attention memory. NO mean-pooling, NO layer-32 tap, NO
# linear-read shortcut (the prior kv_unified.py regressed the AoA collection -> flat lift; this restores it). Compare to
# layer-32-AoA KV-write (+0.033 agr / +0.155 KL).
...
CACHE = '/home/pokazge/checkpoints/native_kv_seq.pt'
_orig = Q5.eager_attention_forward                                               # KV-WRITE patch (validated)
...
def full_msgs(m, t):
    ...
    return ms
@torch.no_grad()
def native_kv_seq(ms, chunk_text):                                               # the chunk tokens' native KEYS+VALUES at TGT layers (per token, mean over heads) -> [n_tok, 2*len(TGT)*hd]
    ...
    for L in TGT:
        lc = cache.layers[L]; feats.append(lc.keys[0, :, -nct:, :].mean(0)); feats.append(lc.values[0, :, -nct:, :].mean(0))  # [nct, hd] each
    return torch.cat(feats, dim=-1).float().cpu()                                # [nct, 2*len(TGT)*hd]
if os.path.exists(CACHE):
    seqs = torch.load(CACHE, weights_only=False)
    for i, m in enumerate(data): m['nkv'] = seqs[i]
    print('loaded cached native KV sequences', flush=True)
else:
    for mi, m in enumerate(data):
        ...
        m['nkv'] = ns
        if mi == 0: print('native KV seq dim=%d (per token)' % ns[0].shape[1], flush=True)
    torch.save([m['nkv'] for m in data], CACHE); print('extracted + cached native KV sequences for %d trajs' % len(data), flush=True)
nkv_dim = data[0]['nkv'][0].shape[1]; MUk = torch.cat([c for m in data for c in m['nkv']], 0).mean(0)
...
print('FAITHFUL unified: Liquid + AoA over native KV (dim=%d) | train=%d test=%d TGT=%s' % (nkv_dim, len(tr), len(te), TGT), flush=True)
class Compressor(nn.Module):                                                     # SAME Liquid + attention-on-attention as the layer-32 compressor; collect reads the NATIVE KV stream
    ...
    def collect(s, C, b):                                                        # AoA: belief b queries over the native KV token sequence C
    ...
    def run(s, m):
        ...
        for t in range(len(m['nkv'])):
            hp = h; a = s.collect(cenk(m['nkv'][t]), b)                           # attention-on-attention over native KV
            for _ in range(2): b = b + (-b + torch.tanh(s.W(b) + a)) / tau / 2    # Liquid ODE
            h = 0.9 * h + 0.1 * b; hs.append(h); pr.append(s.pred(torch.cat([s.cz(m['z'][t]), hp])))
        return torch.stack(pr), torch.stack(hs)
comp = Compressor(nkv_dim, D); copt = torch.optim.Adam(comp.parameters(), lr=1e-3)
...
print('native-KV-AoA Liquid pretrained; held-out next-gist coh %.3f' % st.mean(gp), flush=True)
...
for ep in range(EP):
    ...
    for m in tr:
        ...
        if SMOKE: break
    print('faithful-unified ep %d kl-loss %.3f (n=%d) gk/gv=%s' % (ep, tot / max(n, 1), n, {L: (round(float(gen.gk[str(L)]), 1), round(float(gen.gv[str(L)]), 1)) for L in TGT}), flush=True)
print('\n=== FAITHFUL UNIFIED (Liquid+AoA native read + KV write, held-out) ===', flush=True)
aw, al, klw, kll = [], [], [], []
with torch.no_grad():
    for m in te:
        for t in range(W, len(m['texts'])):
            ...
            if SMOKE: break
        if SMOKE: break
print('  next-token agreement w/ full-context:  window-alone %.3f   window+NATIVE(AoA-read+KV-write) %.3f   (lift %+.3f)' % (st.mean(aw), st.mean(al), st.mean(al) - st.mean(aw)), flush=True)
...
torch.save({'gen': gen.state_dict(), 'comp': comp.state_dict(), 'tgt': TGT, 'nkv': nkv, 'hd': hd, 'minj': MINJ}, '/home/pokazge/checkpoints/kv_unified2.pt')
